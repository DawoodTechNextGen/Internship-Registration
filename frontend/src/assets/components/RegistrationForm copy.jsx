...
const TechnologySelect = ({
  value, // This should be technology ID
  onChange,
  error, // This should be validation error (like "Technology selection is required")
  technologies,
  loading,
  onRetry,
  apiError, // Add this prop for API errors
}) => {
  ...
  // If there's an API error (failed to fetch technologies)
  if (apiError) {
    return (
  ...
  return (
    <CustomSelect
      ...
      icon={Code}
      error={error} // This is the validation error like "Technology selection is required"
      loading={loading}
...
const ExperienceSelect = ({ value, onChange, error }) => {
  // Convert the integer value to the corresponding experience object
  ...
  return (
    <CustomSelect
      label="Experience *"
      value={getSelectedExperience()} // Pass the object, not just the value
      onChange={onChange}
        ...
        <div className="flex-1 min-w-0">
          ...
          <ul className="space-y-1">
            {Object.entries(errors).map(([field, message]) => (
              <li key={field} className="text-red-700 dark:text-red-400 text-sm">
...
const RegistrationForm = () => {
  ...
  const [serverErrors, setServerErrors] = useState({}); // Separate state for server errors
  ...
  const handleChange = (e) => {
    ...
    if (name === "cnic" && selectedCountry?.cnic) {
      ...
      // Clear both client and server errors for this field
      setErrors((prev) => ({
        ...prev,
  ...
  const handleSelectChange = (name, value) => {
    // For technology, we need to handle it specially
    if (name === "technology") {
      // value is the technology object from CustomSelect
      if (value && typeof value === "object") {
        // Store the ID in formData
        setFormData((prev) => ({
          ...prev,
          [name]: value.id, // Store ID instead of name
        }));
        setErrors((prev) => ({
          ...prev,
      ...
      // For experience, store the integer value
      if (value && typeof value === "object") {
        setFormData((prev) => ({
          ...prev,
          [name]: value.value, // Store the integer value (0, 1, 2, 3, 4)
        }));
        setErrors((prev) => ({
          ...prev,
      ...
      // For other select fields (internshipType, etc.)
      let optionValue;
  ...
  const handleSubmit = async (e) => {
    ...
    if (loading) {
      alert("Please wait while technologies are loading...");
      return;
    }
    ...
    try {
      // Find the selected technology object
      const selectedTech = technologies.find(
        (t) => t.id === formData.technology
      );

      // Find the selected experience object for debugging
      ...
      if (error.response) {
        ...
        if (status === 422 && data.errors) {
          // Create a new errors object to show server-side validation errors
          const serverValidationErrors = {};

          // Map server errors to form fields
          Object.keys(data.errors).forEach((field) => {
            // Convert field names from API to form field names
            let formField;
            switch (field) {
              case "mbl_number":
                formField = "whatsapp";
          ...
          // Set the server errors in state (separate from client validation errors)
          ...
          // Show a general alert with the main message
          alert(data.message || "Please fix the errors below and try again.");
        } else if (status === 400) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 px-4 py-8">
      ...
      <div className="w-full max-w-4xl">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-black bg-gradient-to-r from-slate-800 to-slate-600 dark:from-slate-200 dark:to-slate-400 bg-clip-text text-transparent">
            Internship Registration
          </h1>
          <p className="text-slate-600 dark:text-slate-400 mt-2">
            DawoodTech NextGen Pakistan
          </p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <TechnologySelect
                  ...
                  onChange={(value) => handleSelectChange("technology", value)}
